...
- NONSRICT_READ_WRITE - не такая жесткая как READ_WRITE, invalidate данных в кэше (запись нового значения одной транзакцией) происходит с некоторой задержкой, и другие транзакции могут увидеть старое состояние сущности.
- TRANSACTIONAL - используется для распределенных транзакций. Не каждый провайдер поддерживает этот режим. Есть две разные БД, которые находятся на разных машинах и нужно произвести транзакцию в рамках двух различных машин. Hibernate поддерживает, например через atomics с помощью двухфазового комита.

## Query cache

Кэш второго уровня позволяет сохранять запросы.

В случаи сущности в качестве ключа выступает id и класс. А в случае запроса ключем query cache является вся строка запроса.

По сути в кэше сохраняются id, но не сущности, поэтому @Cache нужно ставить над сущностями.

В конфиге нужно прописать использование кэша для запросов
